...
class TestFileStorage_instantiation(unittest.TestCase):
    ...
    def test_permissions(self):
        ...
        self.assertTrue(w)
        x = os.access('models/engine/file_storage.py', os.X_OK)
        self.assertTrue(x)

    def test_fileStorage_from_models(self):
        """test inicialization of storage var in models"""
        self.assertEqual(type(models.storage), FileStorage)
        self.assertEqual(type(model.storage._FileStorage__file_path), str)
        self.assertEqual(type(model.storage._FileStorage__objects), dict)
